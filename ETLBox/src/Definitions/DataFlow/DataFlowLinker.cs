...
namespace ALE.ETLBox.DataFlow
{
    public class DataFlowLinker<TOutput>
    {
        ...
        public IDataFlowLinkSource<TConvert> LinkTo<TConvert>(IDataFlowLinkTarget<TOutput> target)
        {
            ...
            if (!DisableLogging)
                NLogger.Debug(CallingTask.TaskName + $" was linked to {((GenericTask)target).TaskName}", CallingTask.TaskType, "LOG", CallingTask.TaskHash, ControlFlow.ControlFlow.STAGE, ControlFlow.ControlFlow.CurrentLoadProcess?.LoadProcessKey);
        ...
        public IDataFlowLinkSource<TConvert> LinkTo<TConvert>(IDataFlowLinkTarget<TOutput> target, Predicate<TOutput> predicate)
        {
            ...
            if (!DisableLogging)
                NLogger.Debug(CallingTask.TaskName + $" was linked to {((GenericTask)target).TaskName} (with predicate)!", CallingTask.TaskType, "LOG", CallingTask.TaskHash, ControlFlow.ControlFlow.STAGE, ControlFlow.ControlFlow.CurrentLoadProcess?.LoadProcessKey);
            return target as IDataFlowLinkSource<TConvert>;
        }
