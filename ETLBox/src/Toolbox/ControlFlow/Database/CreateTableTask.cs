...
{CreateFinallyComments(TableDefinition)}";
        ...
        private string CreateIdentitySql(ITableColumn col)
        {
            ...
            return ConnectionType switch
            {
                ConnectionManagerType.ClickHouse => "DEFAULT generateUUIDv4()",
                ConnectionManagerType.MySql => "AUTO_INCREMENT",
                ConnectionManagerType.Postgres => $"generated by default as identity (minvalue {col.IdentitySeed ?? 1} increment by {col.IdentityIncrement ?? 1})",
        ...
        private string CreateFinallyComments(TableDefinition table)
            ...
            {
                ConnectionManagerType.SqlServer => string.Join("\r\n",
                table.Columns
                    .Select(c => $@"
    --ColumnDescription
    exec sp_addextendedproperty
        ...
        @level2name = N'{c.Name}';
    ")),
                ConnectionManagerType.Postgres => string.Join(";\n",
                    table.Columns
